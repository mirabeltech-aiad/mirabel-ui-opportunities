...
const TasksTab: React.FC<TasksTabProps> = ({ companyId }) => {
  // Fetch tasks from API with automatic triggering
  const { data: tasksData, isLoading, error, refetch } = useQuery<TasksApiResponse>({
    queryKey: ['tasks', companyId],
    queryFn: async (): Promise<TasksApiResponse> => {
      console.log('TasksTab: Fetching tasks for companyId:', companyId);
      const payload = {
        spName: "uspCDCSyncOpportunityTaskGet",
        isMMDatabase: false,
        spParam: {
          TaskType: "3",
          CustomerId: parseInt(String(companyId)) || 0
        }
      };
      const response = await apiService.post('/services/admin/common/production/executesp/', payload);
      console.log('TasksTab: API response:', response);
  ...
  useEffect(() => {
    if (companyId) {
      console.log('TasksTab: Component mounted/companyId changed, triggering task fetch for:', companyId);
      refetch();
    }
  }, [companyId, refetch]);

  // Transform API data to match existing component structure
  const tasks: Task[] = tasksData?.content?.Data?.Table?.map(task => ({
    id: task.ID,
    title: task.Title,
  ...
  const [prioritySearchTerm, setPrioritySearchTerm] = useState<string>("");

  // Fetch priorities on mount
  useEffect(() => {
    const fetchPriorities = async (): Promise<void> => {
      try {
        console.log('TasksTab: Fetching priorities...');
        const response: PriorityApiResponse = await apiService.get('/services/Admin/Masters/TaskListPriority');
        if (response?.content?.Status === 'Success' && Array.isArray(response.content.List)) {
          setPriorities(response.content.List);
          console.log('TasksTab: Priorities loaded:', response.content.List.length);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const fetchUsers = async (): Promise<void> => {
      try {
        console.log('TasksTab: Fetching users...');
        const response: UserApiResponse = await apiService.get(`/services/User/Accounts/Master/${getCurrentUserId()}/false/true`);
        if (response?.content?.Status === 'Success' && Array.isArray(response.content.List)) {
          setUsers(response.content.List);
          console.log('TasksTab: Users loaded:', response.content.List.length);
        }
      } catch (err) {
  ...
  const addTask = async (): Promise<void> => {
    ...
    try {
      console.log('TasksTab: Adding new task:', newTask);
      ...
      const formatDateForSQL = (date: string | Date): string | null => {
        if (!date) return null;
      ...
      const payload = {
        ...
        spParam: {
          ...
          pCustomerID: parseInt(String(companyId)) || 0,
          pIsTaskComplete: false,
          pSPAction: "SAVE",
          TaskType: "3"
        }
      };
      console.log('TasksTab: Saving task with payload:', payload);
      await apiService.post('/services/admin/common/production/executesp/', payload);
      console.log('TasksTab: Task saved successfully');
      setNewTask({
        title: "",
        description: "",
        assignedTo: "",
