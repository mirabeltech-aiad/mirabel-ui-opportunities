...
interface Customer {
    ...
    company: string;
    firstName: string;
    primaryContact: number | string | boolean;
    displayLabel: string;
    value: string;
}
...
interface SearchError {
    message: string;
    isRetryable: boolean;
    type: 'network' | 'timeout' | 'connection' | 'validation' | 'auth' | 'permission' | 'server' | 'unknown' | 'generic';
}
...
const CustomerSearchBar: React.FC<CustomerSearchBarProps> = ({
    ...
    const parseSearchError = (error: any): SearchError => {
    ...
    const searchCustomers = async (searchText: string): Promise<void> => {
        ...
        try {
            ...
            if (
                ...
                try {
                    const parsedData: CustomerApiData[] = JSON.parse(response.content.JSONContent);

                    if (Array.isArray(parsedData) && parsedData.length > 0) {


                        const customerList: Customer[] = parsedData.map((customer, index) => {
                            const primaryContact = customer.PrimaryContact ?? false;
                            const isPrimary =
                                primaryContact === 1 ||
                                primaryContact === "1" ||
                                primaryContact === true;
                            ...
                            return {
                                ...
                                displayLabel: `${customer.Company || "Unknown Company"} (${customer.FirstName || "Unknown Contact"
                                    }${primaryContactText})`,
