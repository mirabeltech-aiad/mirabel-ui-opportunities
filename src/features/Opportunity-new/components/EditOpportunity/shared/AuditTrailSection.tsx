...
interface AuditEntry {
  ...
  newValue: string;
  userId: string;
  type: 'status' | 'assignment' | 'field' | 'system';
}
...
const AuditTrailSection: React.FC<AuditTrailSectionProps> = ({
  ...
  const [historyData, setHistoryData] = useState<AuditEntry[]>([]);
    ...
    const fetchOpportunityHistory = async () => {
      ...
      try {
        ...
        if (response && response.content && response.content.Data && response.content.Data.Audit) {
          ...
          const transformedData: AuditEntry[] = auditData.map((item: any, index: number) => ({
            id: `history-${index}`,
  ...
  const getChangeTypeFromField = (fieldName: string): AuditEntry['type'] => {
  ...
  const getTypeIcon = (type: AuditEntry['type']): React.ReactNode => {
    switch (type) {
      case 'status':
  ...
  const getTypeColor = (type: AuditEntry['type']): string => {
    switch (type) {
      case 'status':
  ...
  const renderChangeDescription = (entry: AuditEntry): React.ReactNode => {
