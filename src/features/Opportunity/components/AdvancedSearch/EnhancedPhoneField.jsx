...
const EnhancedPhoneField = ({ 
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      const isOutsidePortal = !event.target.closest('[data-phone-dropdown]');
  ...
  const handleAddCustomValue = () => {
    ...
    if (trimmedValue) {
      // Store plain value directly (no SW=...~ formatting for custom values)
      const plainValue = trimmedValue;
      
      // Check if this value already exists
      const valueExists = currentValues.some(val => {
        if (val === plainValue) return true;
      ...
      if (!valueExists) {
        ...
        const newValues = [...customValues, plainValue];
        onChange(newValues);
        setInputValue("");
        setIsDropdownOpen(false);
      } else {
  ...
  return (
    <div className="space-y-2">
      ...
      {/* <p className="text-xs text-gray-500">
        Type custom phone numbers or select predefined options. Selecting "Is Empty" or "Is Not Empty" will clear custom text.
      </p> */}
    </div>
  );
};
