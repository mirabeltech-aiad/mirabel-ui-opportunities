...
const EnhancedOpportunityNameField = ({ 
  ...
  const parseValues = (val) => {
    ...
    // Check if it's in the format "value~SW=value~" (multiple values concatenated)
    if (strVal.includes('~SW=')) {
      return strVal.split('~SW=').map((part, index) => {
        if (index === 0) {
          // First part might have a prefix, extract just the value
          const cleanPart = part.replace(/^.*=/, '');
          return cleanPart ? `SW=${cleanPart}~` : null;
        } else {
          // Subsequent parts are already in SW=format
          return `SW=${part.replace(/~$/, '')}~`;
        }
      }).filter(Boolean);
    }
    
    // Handle single SW=value~ format
    if (strVal.startsWith('SW=') && strVal.endsWith('~')) {
      return [strVal];
    }
    ...
    // Handle plain text (convert to SW=format)
    return [`SW=${strVal}~`];
  };
    ...
    if (trimmedValue) {
      ...
      const alreadyExists = currentValues.some(val => {
        ...
        if (val.startsWith('SW=') && val.endsWith('~')) {
          const existingText = val.slice(3, -1);
          return existingText === trimmedValue;
        }
  ...
  const handleContainerClick = () => {
    ...
    const isCurrentValuePredefined = currentValues.length === 1 && 
      predefinedOptions.some(option => option.value === currentValues[0]);
