...
const OpportunityStatsSection = ({ opportunityId }) => {
  ...
  const { data: statsData, isLoading, error } = useQuery({
    queryKey: ['opportunityStats', opportunityId],
    queryFn: async () => {
      const payload = {
        spName: "uspCDCSyncsGetStats",
        isMMDatabase: false,
        spParam: {
  ...
  const processStatsData = () => {
    ...
    const generalStats = {
      ...
      averageContactTimeframe: "2.3 days", // Keep original calculation
      totalProposals: 3, // Keep original value
      daysUntilClose: daysUntilCloseInfo,
      totalContacts: 6 // Keep original value
    };
    ...
    const stageTimings = stageData.map((stage, index) => {
      const isCurrentStage = index === stageData.length - 1; // Last stage is current
  ...
  const { generalStats, stageTimings, totalDays, currentStage, averageStageTime, longestStage } = processStatsData();
