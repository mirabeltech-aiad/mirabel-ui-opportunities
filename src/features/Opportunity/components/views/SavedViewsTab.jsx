...
import { ScrollArea } from "@/features/Opportunity/components/ui/scroll-area";
...
const SavedViewsTab = ({ savedViews, onLoadView, onDeleteView, onUpdateView, onSaveAsView, onEditView, activeViewId }) => {
  ...
  const handleEditStart = (view) => {
    ...
    setEditName(view.NameOfView);
  };

  const handleEditSave = () => {
    onUpdateView(editingId, editName);
    setEditingId(null);
    setEditName("");
  };
      ...
      <ScrollArea className="flex-1 pr-3">
        {validSavedViews.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400 text-xs mt-1">Create your first view using the Add View tab</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {globalViews.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-semibold text-gray-800 uppercase tracking-wide">Global Views</h3>
            ...
            {myViews.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-semibold text-gray-800 uppercase tracking-wide">My Views</h3>
                  <div className="flex-1 h-px bg-gray-200"></div>
                </div>
                <div className="space-y-1">
            ...
            {myViews.length === 0 && globalViews.length === 0 && validSavedViews.length > 0 && (
              <div>
                <div className="text-center py-6">
                  <div className="w-10 h-10 bg-gray-50 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <div className="w-5 h-5 bg-gray-200 rounded"></div>
                  </div>
                  <p className="text-gray-500 text-sm">Views found but could not be categorized</p>
                  <p className="text-gray-400 text-xs mt-1">Please check the console for debugging information</p>
                </div>
              </div>
            )}
            ...
            {myViews.length === 0 && globalViews.length > 0 && (
              <div>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-gray-500 text-sm">No custom views created yet</p>
                  <p className="text-gray-400 text-xs mt-1">Use the Add View tab to create your first custom view</p>
                </div>
              </div>
            )}
          </div>
