...
For complete troubleshooting guidance, see the full TROUBLESHOOTING.md file in the project root.`;
  ...
  const renderDocumentContent = (docId) => {
    switch (docId) {
      case 'contributing':
        return (
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold">
                  {documentationFiles.find(doc => doc.id === activeDoc)?.title}
                </h3>
                <p className="text-sm text-gray-500">
                  {documentationFiles.find(doc => doc.id === activeDoc)?.name}
                </p>
              </div>
