...
const KanbanCard = ({
  ...
  React.useEffect(() => {
    const dragId = getOpportunityDragId(opportunity, index);
    registerDraggableId(dragId);

    return () => {
      unregisterDraggableId(dragId);
    };
  }, [opportunity, index]);
  ...
  const draggableId = getOpportunityDragId(opportunity, index);

  // Ensure we have a valid draggable ID before rendering
