...
export const buildExecutiveDashboardParams = (
  ...
  // Set dynamic UserID from session with proper fallback
  try {
    const { userId } = require('@/services/httpClient');
    defaultParams.UserID = userId || getCurrentUserId(); // Use dynamic user ID
    console.log('Using UserID:', defaultParams.UserID);
  } catch (error) {
    console.warn('Could not import userId from httpClient, using session user ID');
    defaultParams.UserID = getCurrentUserId(); // Use session user ID as fallback
  }
