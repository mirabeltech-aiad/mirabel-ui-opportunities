...
class RepPerformanceService {
  ...
  async getRepPerformanceData(dateRange = 'last-12-months', selectedRep = 'all', selectedStatus = 'all', selectedProduct = 'all', selectedBusinessUnit = 'all') {
    try {
      ...
      console.log('Rep performance API response:', response);
      return response;
    } catch (error) {
  ...
  transformToTeamData(apiData) {
    ...
    return teamData.map(rep => ({
      id: rep.RepID || Math.random(),
      name: rep.RepName || 'Unknown Rep',
      initials: rep.RepName ? rep.RepName.split(' ').map(n => n[0]).join('') : 'XX',
      deals: rep.WonOpportunities || 0,
      ...
      quotaAttainment: rep.QuotaAttainment || 0,
  ...
  transformToMonthlyData(apiData) {
    ...
    const repGroups = monthlyData.reduce((acc, item) => {
      ...
      if (!acc[repName]) {
        acc[repName] = { rep: repName };
      }
      
      const monthKey = item.MonthName?.substring(0, 3) || 'Unknown';
      acc[repName][`${monthKey}_created`] = item.OpportunitiesCreated || 0;
      acc[repName][`${monthKey}_won`] = item.OpportunitiesWon || 0;
      acc[repName][`${monthKey}_lost`] = item.OpportunitiesLost || 0;
      
      return acc;
    }, {});
    
    return Object.values(repGroups);
  }
