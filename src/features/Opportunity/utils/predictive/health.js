...
export const analyzePipelineHealth = (opportunities, availableStages = []) => {
  ...
  const stageVelocity = calculateAdvancedStageVelocity(closedOpportunities, availableStages);
  
  // Multi-dimensional conversion analysis
  const conversionRates = calculateAdvancedConversionRates(opportunities, availableStages);
...
const calculateAdvancedStageVelocity = (closedOpportunities, availableStages = []) => {
  // Use live stages if available, fallback to standard stages
  const stages = availableStages.length > 0 
    ? availableStages.map(s => s.name || s.standardStage || s)
...
const calculateAdvancedConversionRates = (opportunities, availableStages = []) => {
  // Use live stages if available, fallback to standard stages
  const stages = availableStages.length > 0 
    ? availableStages.map(s => s.name || s.standardStage || s)
    : ['Prospecting', 'Qualification', 'Needs Analysis', 'Proposal', 'Negotiation'];
