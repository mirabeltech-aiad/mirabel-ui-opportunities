...
export const mapStageToStandard = (apiStage) => {
  ...
  if (STAGE_MAPPING[apiStage]) {
    return STAGE_MAPPING[apiStage];
  }
  
  // If no exact match found, return the stage as-is for now
  // This allows for dynamic stages from API that aren't in our mapping
  console.log(`Stage "${apiStage}" not found in mapping, using as-is`);
  return apiStage;
};
