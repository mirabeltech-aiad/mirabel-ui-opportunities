...
import { settingsMeta,separatorOptions, pickupOptions } from "../helpers/configData";
    ...
    <Card className="mb-6">
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between p-2">
          ...
          <Switch
            checked={!!state["IsESignature"]}
            onCheckedChange={(checked) => {
              handleToggle("IsESignature");
              if (!checked) {
                handleInput("RsKey", "");
              }
            }}
        ...
        {state.IsESignature && (
          <div className="flex items-center justify-between p-2 border-b">
            <div className="flex flex-col items-start">
              <Label className="mb-1 text-base font-semibold" htmlFor="RsKey">
                eSignature API Key
              </Label>
              <p className="text-sm text-left text-muted-foreground">
                Note : If left blank, the default API key of Mirabel
                Technologies will be used while sending documents.
              </p>
            </div>
            <Input
              id="RsKey"
              value={state.IsESignature ? state["RsKey"] : ""}
              onChange={(e) => handleInput("RsKey", e.target.value)}
              className="w-64"
            />
            <Button
              onChange={(e) => handleInput("RsKey", e.target.value)}
              className="w-64"
              disabled={!state.RsKey}
            >
              Validate Account
            </Button>
          </div>
          ...
          <div className="flex flex-row flex-wrap items-center gap-4">
            <div className="flex flex-row items-center min-w-[420px]">
              <select
                ...
                value={
                  state.LoggedInRepChoiceProposal === false ? "false" : "true"
                }
                onChange={(e) =>
                  handleInput(
              ...
              <span className="ml-2 text-sm text-muted-foreground">
                for the default sales rep that is assigned to a new item on{" "}
                <b>proposals</b>.
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-row flex-wrap items-center gap-4">
            <div className="flex flex-row items-center min-w-[420px] mt-2 md:mt-0">
              <select
                ...
                onChange={(e) =>
                  handleInput("LoggedInRepChoice", e.target.value === "true")
                }
              >
                <option value="true">Logged in Rep</option>
                <option value="false">Rep on Client Account</option>
          ...
          <div className="flex flex-col items-start">
            ...
            <div className="flex flex-wrap text-sm text-muted-foreground">
              ...
              <Input
                ...
                min={0}
                className="w-16 mx-2"
                value={state.IssueYearCheckBoxEnd || ""}
                onChange={(e) =>
                  handleInput("IssueYearCheckBoxEnd", e.target.value)
                }
              />{" "}
              ...
              <Input
                ...
                onChange={(e) =>
                  handleInput("IssueYearCheckBoxStart", e.target.value)
                }
              />{" "}
        ...
        <div className="flex items-center justify-between p-2 border-b">
          <div className="flex flex-col items-start">
            ...
            <div className="flex flex-wrap text-sm text-muted-foreground">
              Do not allow proposals to be created for clients that have an
              unpaid balance more than
              <Input
                type="number"
                min={0}
                className="w-16 mx-2"
                ...
                onChange={(e) =>
                  handleInput("CheckCreditLimitOnProposeDays", e.target.value)
                }
              />{" "}
              days past due that are over their credit limit. The credit limit
              is set on the "Details" page of a contact.
              <br />
              <br />
              <span className="pl-2 text-sm text-muted-foreground">
                (*Admin users can still create proposals.)
              </span>
            </div>
          </div>
          <Switch
            checked={state.CheckCreditLimitOnPropose}
        ...
        <div className="flex items-center justify-between p-2 border-b">
          <div className="flex flex-col items-start">
            ...
            <div className="flex flex-wrap text-sm text-muted-foreground">
              Do not allow orders/insertions to be added into the system for
              clients that have an unpaid balance more than
              <Input
                type="number"
                min={0}
                className="w-16 mx-2"
                value={state.CheckCreditLimitOnAddDays} // default value is 0
                onChange={(e) =>
                  handleInput("CheckCreditLimitOnAddDays", e.target.value)
                }
              />{" "}
              days past due that are over their credit limit. The credit limit
              is set on the "Details" page of a contact.
              <br />
              <span className="pl-2 text-sm text-muted-foreground">
                (*Admin users can still create insertions/orders.)
              </span>
            </div>
          </div>
          <Switch
        ...
        <div className="flex items-center justify-between p-2 border-b">
          <div className="flex flex-col items-start">
            ...
            <div className="flex flex-wrap text-sm text-muted-foreground">
              ...
              />{" "}
              days past due that are over their credit limit will be displayed
              in red on the "Sales Runsheet" report.
            </div>
          </div>
          <Switch
        ...
        <div className="flex items-center justify-between p-2">
          <div className="flex flex-col items-start">
            ...
            <p className="text-sm text-left text-muted-foreground">
              ...
              items, editing existing items, converting proposals to orders or
              copying existing orders. Inventory limits are set by
              product/issue/adsize or product/adsize for products with no issues
              (i.e. digital only).
            </p>
            <br />
            <p className="text-sm text-left text-muted-foreground">
              If "Warning" is selected, a window with a warning will show before
              any new items are created, converted or copied; showing a summary
              of the inventory numbers with a link to the inventory report and a
              button to proceed with the entry despite the warning.
            </p>
            <br />
            <p className="text-sm text-left text-muted-foreground">
              If "Stop" is selected, the button to "Proceed" with the entry will
              not be available and the item cannot be
              created/copied/converted/etc. *Admin users can still add items
              even if they "Stop Processing" option is checked.
            </p>
          </div>
        </div>
